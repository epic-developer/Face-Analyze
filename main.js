...
async function query(url, level) {
    ...
    if (fileSelected.length > 0) {
        ...
        fileReader.onload = async function (faceURL) {
            ...
            $.getJSON('https://api.allorigins.win/get?url=' + encodeURIComponent(url), async function (data) {
                var images = [];
                    ...
                    try {
                        ...
                        if (item2 != '' && (item2.includes('.jpg') || item2.includes('.png'))) {
                            if (item2.includes('https://') || item2.includes('http://')) {
                                images.push('https://face-analyze.davidfahim.repl.co/get/' + encodeURIComponent(item2));
                                //images.push(item2);
                            }
                            /*else {
                                var url2;
